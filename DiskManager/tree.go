...
// helps in delete logic maintaining function signature
...
func (e *DeleteKeyError) Error() string {
	if e.Err != nil {
		return fmt.Sprintf("%s:(error parameters - is underfill %t, is leaf %t, is borrow possible %t)",
			e.Err.Error(), e.IsUnderfill, e.IsLeaf, e.BorrowPossible)
	}
	return fmt.Sprintf("is underfill %t, is leaf %t, borrow possible %t", e.IsUnderfill, e.IsLeaf, e.BorrowPossible)
}
...
func (t tree) BorrowLeaf(currPageAddr int32, currIndex int32, leftChildAddr int32, rightChildAddr int32) error {
...
func (t tree) Delete(key int32) error {
	return fmt.Errorf("tree:delete not implemented")
}
